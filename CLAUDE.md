...
- `docs/01-product/DESIGN.md` — the authoritative design: the extraction boundary (each pillar mapped to the `gitdone` source files in `~/PycharmProjects/gitdone/app/src` that will be lifted), locked decisions, planned public API, and phasing. **Read it before doing P1 work.**

Do not describe gitcore as if the library exists. The real lift (P1) is pending.
...
- **Four pillars, decoupled.** verify → sequence → git ledger → email triggers. Each maps to specific `gitdone` modules (see DESIGN.md). The orchestrator (`receive.js`) is **app glue and is NOT lifted** — gitcore exposes the primitives (+ an optional `createReceiver()` with policy hooks); each consumer writes its own thin glue.
...
- **Generic workflow only** in v1. Crypto declaration/attestation, strict signing, revoke, thresholds stay in `gitdone` as policy on gitcore hooks — do **not** drag them into the core when lifting `completion.js` (extract only its workflow subset).
...
This project follows the standards in **`.claude/memory/AGENT_RULES.md`** (POC-first, strict dependency hierarchy vanilla→stdlib→external, lightweight over complex, open-source only, Testing-Trophy). They are mandatory, not suggestions. Notably for gitcore: keep the dependency surface tiny (target ≤3 runtime deps, matching gitdone/knowless), and graduate the POC by rewriting — never shipping it.

## Relationship to gitdone
**`gitdone`** (`~/PycharmProjects/gitdone`) is the source of the extraction and gitcore's eventual first consumer — P2 refactors it to depend on gitcore, reimplementing its crypto modes as policy on gitcore's hooks.
