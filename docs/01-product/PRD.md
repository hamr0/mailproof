...
Pure ESM (`import { create } from 'mailproof'`): `create({ dataDir, domain,
...
| 8.13 | **TypeScript source** | Source stays vanilla JS + JSDoc; no `.ts`, no consumer build step. The shipped `.d.ts` are *generated* from the JSDoc by a dev-only `tsc`, emitted to **`./types/` (git-ignored), and built fresh on publish** (`prepublishOnly`) — never committed, so no stored artifact can drift from the JSDoc. `tsc` runs `checkJs` + **`strictNullChecks`** (NOT full `strict`: full strict is ~95% `noImplicitAny` annotation-noise and ~5% real null-safety — `strictNullChecks` keeps the 5%, drops the noise); CI gates on it (`tsc --noEmit` on push/PR + before publish). Null-safety findings are fixed with minimal behaviour-preserving guards — never `!`, `as any`, or `@ts-ignore`. JSDoc is the single source of truth. This is the suite-wide library types contract (`bareagent`/`knowless`; see `LIBRARY_CONVENTIONS.md`): TS consumers get a checked surface while JS authoring + the runtime dep count are unchanged. |
