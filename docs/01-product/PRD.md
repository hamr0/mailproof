...
**Status:** Pre-library — P0 (POC) complete; **P1 (lift) in progress.** Lifted so far: verify (`classifier.js`), the inbound decoder (`parse.js` — `authenticateMessage` over `mailauth` + `parseMessage` over `mailparser`, m7a; mailproof's first **2 runtime deps**, both required because verifying/parsing untrusted mail is security-critical), sequence routing (`router.js`, trimmed to kernel tags), inbound preprocessing (`prefilter.js`, `envelope.js`), outbound triggers (`outbound.js`, config-injected), the full storage/ledger pillar (`event-store.js` + `event-mutex.js` + `gitrepo.js` + optional `ots.js`, factory-injected; the git ledger talks to the `git` binary via `child_process`, no `simple-git`), **both sequencing engines** (`completion.js` — workflow subset, one `dependsOn` model — and `crypto.js` — the parameterized sign-off engine, m6.7), and the document notary (`notary.js` — `verifyDocument` + the canonical `hashDocument`, with the mandatory inbound **auto-hash capture** now wired through `parseMessage`, §4.1). Still pending: the `create()` / `ingest()` assembly that composes the pillars (m7b — in progress; m7b-1 landed the accept-with-flag commit fields `counted`/`count_reason` + lifted `commitCompletion`; `createEvent` + the `attest+` route (m7b-2) and `ingest()`/`create()` (m7b-3) remain).
**Owner:** hamr0
