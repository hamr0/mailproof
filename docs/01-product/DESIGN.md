...
wants it. `gitcore` is that engine as a standalone vanilla library; `gitdone`
...
- **P2 — Reconverge:** refactor `gitdone` to depend on `gitcore`, reimplementing crypto modes as policy on gitcore hooks. Other downstream services adopt gitcore where a multi-party, email-grounded confirmation chain is needed.
